<?php
// /public/clients_online.php
// Show ONLY online clients, using live PPP Active from MikroTik
...
/* Live toggle (1=read from MikroTik, 0=DB fallback) */
...
/* ================== LIVE online usernames (PPP Active) ================== */
$activeUsers = []; // set-like: [pppoe_id => 1]
...
if ($live) {
    if (!empty($activeUsers)) {
        $in = implode(',', array_fill(0, count($activeUsers), '?'));
        $sql_base .= " AND c.pppoe_id IN ($in)";
    ...
    if (in_array('is_online', $clientCols, true)) {
...
<div class="container-fluid p-3 p-md-4">
  ...
  <div class="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-3">
    <div class="d-flex align-items-center gap-3">
      <h4 class="mb-0">Online Clients</h4>
    ...
    <div class="d-flex align-items-center gap-2">
      ...
      <a href="clients_offline.php" class="btn btn-outline-secondary btn-sm">
        <i class="bi bi-wifi-off"></i> Offline Clients
      </a>
    </div>
  </div>
  ...
  <div class="table-responsive" style="background:#fff;padding:12px;border-radius:12px;box-shadow:0 2px 10px rgba(0,0,0,.08)">
    <table class="table table-hover table-striped table-sm align-middle">
      ...
      <tbody>
        <?php if (empty($rows)): ?>
          <tr><td colspan="7" class="text-center text-muted">No online clients</td></tr>
        <?php else: foreach($rows as $c): ?>
          <tr>
            <td>
              <span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:#198754;margin-right:6px"></span>
...
// বাংলা নোট: ট্যাব hidden থাকলে রিফ্রেশ স্কিপ করি
(function(){
